...
Commands::
    ...
    inklang reverse <input.svg>  --out <output.yaml>
...
via Inkscape; ``reverse`` converts an SVG back into InkLang YAML.
"""
...
from .reverse_parser import svg_to_yaml
...
@cli.command()
@click.argument("input_svg", type=click.Path(exists=True, dir_okay=False))
@click.option("--out", "out_path", required=False, help="Output .yaml path (optional).")
def reverse(input_svg: str, out_path: str | None) -> None:
    """Convert an SVG file back into InkLang YAML.
    
    Parses an SVG (hand-drawn or generated by InkLang) and outputs a declarative
    YAML design that can be re-rendered. The YAML is printed to stdout unless
    --out is specified.
    """
    try:
        yaml_str = svg_to_yaml(input_svg, output_path=out_path)
        if not out_path:
            click.echo(yaml_str)
        else:
            click.echo(f"Wrote {out_path}")
    except Exception as exc:
        raise InklangError(f"Failed to reverse SVG {input_svg}: {exc}")
